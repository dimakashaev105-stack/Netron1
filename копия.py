...
def handle_db_document(message):
    try:
        ...
        # УСИЛЕННАЯ ПРОВЕРКА ЦЕЛОСТНОСТИ БАЗЫ
        try:
            ...
            essential_columns = ['user_id', 'balance', 'username']
        ...
        # ЗАМЕНА БАЗЫ ДАННЫХ
        try:
            # Закрываем все соединения с текущей базой
            time.sleep(1)
            ...
            response_msg += f"💳 Чеков: {checks_count}\n"
