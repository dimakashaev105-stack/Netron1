    ...
    try:
        ...
        if not top_users:
            ...
            for i, user in enumerate(top_users):
                ...
                # Очищаем display_name от лишнего HTML/ссылок
                display_name = str(display_name).strip()
                
                # Если это юзернейм (начинается с @), делаем ссылку
                if display_name.startswith('@'):
                    # Убираем @ для ссылки
                    username = display_name[1:]
                    # Формируем HTML ссылку
                    display_html = f'<a href="https://t.me/{username}">{display_name}</a>'
                # Если это просто имя, проверяем есть ли nickname в БД
                else:
                    # Получаем nickname из базы
                    conn = get_db_connection()
                    cursor = conn.cursor()
                    ...
                    if user_data and user_data[0] and user_data[0].strip():
                        # Есть кастомный nickname
                        nickname = user_data[0].strip()
                        # Проверяем есть ли username для ссылки
                        username = user_data[1] if user_data[1] else None
                        
                        if username:
                            # Если есть username, делаем ссылку на него
                            display_html = f'<a href="https://t.me/{username}">{nickname}</a>'
                        else:
                            # Если нет username, просто показываем nickname
                            display_html = nickname
                    else:
                        # Нет nickname, показываем то что есть
                        if user_data and user_data[1]:  # Есть username
            ...
            if mode == 'balance':
                ...
                if user_data:
                    ...
                    if nickname and nickname.strip():
                        ...
                        # Получаем first_name если нет ничего
...
def handle_coin_game_minimal(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "⚡")
# === УНИВЕРСАЛЬНЫЙ ОБРАБОТЧИК ИГР С РЕДАКТИРОВАНИЕМ СООБЩЕНИЙ ===
user_game_messages = {}  # {user_id: {"chat_id": X, "message_id": X}}
...
def handle_games_button(message):
    ...
    try:
        ...
        user_id = message.from_user.id
        if user_id in user_game_messages:
            try:
                # Пытаемся отредактировать существующее сообщение
                bot.edit_message_text(
                    get_games_menu_text(),
                    user_game_messages[user_id]["chat_id"],
                    user_game_messages[user_id]["message_id"],
                    reply_markup=create_casino_main_keyboard(),
                    parse_mode='Markdown'
                )
                bot.answer_callback_query(user_id, "✅ Меню обновлено")
                return
            except:
                # Если не получается редактировать, удаляем старую запись
                del user_game_messages[user_id]
...
def casino_callback_handler(call):
    """Обработка колбэков информации об играх с сохранением сообщения"""
    try:
        if is_spam(call.from_user.id):
            bot.answer_callback_query(call.id, "⏳ Слишком быстро! Подождите немного.")
            return
...
def cleanup_game_messages():
    """Очищает старые записи о сообщениях игр"""
    while True:
        time.sleep(300)  # Каждые 5 минут
        current_time = time.time()
        to_remove = []
        
        for user_id, msg_data in list(user_game_messages.items()):
            if current_time - msg_data.get("timestamp", current_time) > 3600:  # 1 час
                to_remove.append(user_id)
...
# Запускаем очистку в отдельном потоке
game_cleanup_thread = threading.Thread(target=cleanup_game_messages, daemon=True)
game_cleanup_thread.start()
