...
def handle_snow_work(message):
    try:
        ...
        # ПРОВЕРКА 1: Есть ли активная работа?
        cursor.execute('''
            SELECT snow_job_end_time, snow_territory, snow_job_total 
            FROM users 
            WHERE user_id = ? AND snow_job_end_time IS NOT NULL
        ''', (user_id,))
        
        job_data = cursor.fetchone()
        current_time = datetime.now()
        
        if job_data and job_data[0]:
            # Есть активная работа, проверяем время окончания
            job_end = None
            ...
            if job_end and current_time < job_end:
                ...
                elapsed_seconds = (total_squares / squares_per_minute * 60) - time_left
        ...
        # ПРОВЕРКА 2: Если работа закончилась, начисляем деньги
        if job_data and job_data[0]:
            if job_end and current_time >= job_end:
                # Работа завершена, начисляем деньги
                total_squares = job_data[2] or 0
                ...
                complete_text += "⚡ Можете начинать новую работу!"
                
                bot.send_message(message.chat.id, complete_text)
                
                # После завершения работы сразу проверяем отдышку
                # (пропускаем и переходим к проверке отдышки ниже)
        
        # ПРОВЕРКА 3: Проверяем отдышку (теперь всегда проверяем после завершения)
        ...
        message_text += f"⏳ Отдышка после: {cool_minutes}:{cool_seconds:02d}\n\n"
...
def schedule_snow_completion(user_id, chat_id, squares, earnings, territory, work_seconds):
    ...
    def complete_job():
        try:
            ...
            # ПРОВЕРКА: Не была ли работа уже завершена вручную?
            cursor.execute('''
                SELECT snow_job_total, snow_job_end_time 
                FROM users WHERE user_id = ?
            ''', (user_id,))
            
            job_data = cursor.fetchone()
            
            # Если работы нет или она уже завершена, ничего не делаем
            if not job_data or not job_data[0] or not job_data[1]:
                conn.close()
                return
            ...
            # Отправляем уведомление (если чат доступен)
            try:
                ...
                pass  # Пользователь заблокировал бота или чат недоступен
    ...
    try:
        ...
        if job_end:
            ...
            if job_end_time and current_time < job_end_time:
                ...
                # Время вышло, но работа не завершена
                # Начисляем деньги
                ...
                status_text += "⚡ Можно начинать новую работу!"
