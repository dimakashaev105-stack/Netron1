...
def handle_reset_all(message):
    """Моментально обнулить балансы, вклады, чеки и дома"""
    try:
        if not is_admin(message.from_user.id):
            return
        
        # Отправляем сообщение о начале
        status_msg = bot.send_message(message.chat.id, "🔄 Обнуляю...")
        
        conn = get_db_connection()
        cursor = conn.cursor()
        
        # Получаем статистику до обнуления
        ...
        # 1. Обнуляем балансы и вклады
        cursor.execute('UPDATE users SET balance = 0, bank_deposit = 0')
        
        # 2. Сбрасываем last_interest_calc
        ...
        # 4. Удаляем все дома пользователей
        cursor.execute('DELETE FROM user_houses')
        
        # 5. Очищаем магазин домов
...
💚 *Елки и видеокарты сохранены*
        ...
        # Логируем действие
        logging.info(f"Админ {message.from_user.id} обнулил балансы, вклады, чеки, дома и магазин")
