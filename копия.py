...
# === СИСТЕМА КУРЬЕРА (ИСПРАВЛЕННАЯ ВЕРСИЯ) ===
...
@bot.message_handler(func=lambda message: message.text.lower() == 'курьерстат')
def handle_courier_stats_cmd(message):
    user_id = message.from_user.id
    stats = get_courier_stats(user_id)
    level_data = COURIER_LEVELS.get(stats["level"], COURIER_LEVELS[1])
    
    msg = f"📊 *СТАТИСТИКА КУРЬЕРА*\n\n"
    msg += f"• Уровень: {level_data['name']}\n"
    msg += f"• Доставок: {stats['deliveries']}\n"
    msg += f"• Заработано: {format_balance(stats['earned'])}❄️\n"
    msg += f"• Опыт: {stats['xp']}/{level_data['xp_needed']}\n"
    
    if user_id in COURIER_JOBS:
        job = COURIER_JOBS[user_id]
        deliveries_left = level_data["deliveries"] - job["done"]
        msg += f"\n🚚 *Активная смена:*\n"
        msg += f"• Осталось: {deliveries_left} доставок\n"
        msg += f"• Заработано: {format_balance(job['earnings'])}❄️\n"
    
    if stats["cooldown"] > 0:
        current_time = time.time()
        if current_time < stats["cooldown"]:
            time_left = int(stats["cooldown"] - current_time)
            minutes = time_left // 60
            seconds = time_left % 60
