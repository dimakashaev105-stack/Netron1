...
def get_roulette_photo(winning_number):
    ...
    images_dir = os.path.join(current_dir, 'roulette_images')
    if not os.path.exists(images_dir):
        os.makedirs(images_dir)
        logging.info(f"Создана папка для изображений: {images_dir}")
    
    # Проверяем разные варианты путей
    possible_locations = [
        current_dir,                     # Текущая директория скрипта
        images_dir,                      # Папка roulette_images
        '/app',                          # Docker путь
        os.path.join(current_dir, 'images'),  # Папка images
        '.',                             # Текущая рабочая директория
    ]
    
    formats = ['.png', '.jpg', '.jpeg', '.PNG', '.JPG', '.JPEG']
    
    # Сначала проверяем в разных местах
    for location in possible_locations:
        for fmt in formats:
            ...
            if os.path.exists(filepath):
                logging.info(f"✅ Найден файл изображения: {filepath}")
                return filepath
    
    # Если файл не найден, пробуем скачать или создать стандартные
    logging.warning(f"Файл для числа {winning_number} не найден")
    return None
# Обработчик рулетки
...
def handle_claim_bonus(call):
    ...
    try:
        ...
        try:
            ...
            # Проверяем время через БД с блокировкой
            conn = get_db_connection()
            cursor = conn.cursor()
            ...
            # Блокируем запись пользователя для чтения с обновлением
            cursor.execute('SELECT last_bonus FROM users WHERE user_id = ? FOR UPDATE', (user_id,))
            result = cursor.fetchone()
