...
# =============== МИНИМАЛИСТИЧНАЯ ВЕРСИЯ КАЗИНО ===============

@bot.message_handler(func=lambda message: message.text.lower() == 'Игры')
def handle_casino_mini(message):
    """Минималистичное меню казино"""
    markup = InlineKeyboardMarkup(row_width=3)
...
*💰 Суммы:*
`100` `1к` `10к` `100к` `1кк` `1ккк`
    """
    
    bot.send_message(message.chat.id, casino_mini_text, reply_markup=markup, parse_mode='Markdown')

# =============== ИНФОРМАЦИЯ ПО КАЖДОЙ ИГРЕ ===============

@bot.callback_query_handler(func=lambda call: call.data.startswith('casino_'))
def casino_info_handler(call):
...
*Шанс:* 50%
    ...
    if game_type in game_info:
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("🎰 Все игры", callback_data="casino_all"))
        ...
        handle_casino_mini(call.message)
