...
# =============== УПРОЩЕННАЯ КНОПКА "ЧИСТКА СНЕГА" ===============
...
def handle_snow_work(message):
    try:
        ...
        # 1. ПРОВЕРЯЕМ: Есть ли активная работа?
        cursor.execute('''
        ...
        if job_data and job_data[0]:  # Есть время окончания работы
            job_end = None
            ...
            if isinstance(job_data[0], str):
                try:
                    ...
                    # Если ошибка парсинга, считаем работу завершенной
                    job_end = current_time
            elif isinstance(job_data[0], (int, float)):
                job_end = datetime.fromtimestamp(job_data[0])
            else:
                job_end = current_time
            
            # Проверяем, не завершилась ли работа
            if job_end and current_time < job_end:
                ...
                total_minutes = total_squares / squares_per_minute
                elapsed_minutes = (total_minutes * 60 - time_left) / 60
                progress = min(int(elapsed_minutes * squares_per_minute), total_squares)
                earned = progress * 5
                
                status_text = "❄️ УЖЕ РАБОТАЕТЕ!\n\n"
                status_text += f"📍 Место: {job_data[1] or '❄️ Зимняя территория'}\n"
                status_text += f"📐 Объем: {total_squares} м²\n\n"
                ...
                status_text += "🔄 Дождитесь окончания текущей работы!"
                
                bot.send_message(message.chat.id, status_text)
                conn.close()
                return
            else:
                # Время работы вышло - ЗАВЕРШАЕМ РАБОТУ
                total_squares = job_data[2] or 0
                earnings = total_squares * 5
                
                # Начисляем деньги
                cursor.execute('UPDATE users SET balance = balance + ? WHERE user_id = ?', 
                             (earnings, user_id))
                
                # Увеличиваем счетчик работ
                cursor.execute('UPDATE users SET snow_jobs_count = snow_jobs_count + 1 WHERE user_id = ?', 
                             (user_id,))
                ...
                complete_text = "✅ РАБОТА ЗАВЕРШЕНА!\n\n"
                complete_text += f"📍 Место: {job_data[1] or '❄️ Зимняя территория'}\n"
                complete_text += f"📐 Очищено: {total_squares} м²\n"
                complete_text += f"💰 Заработано: {earnings}❄️\n"
                complete_text += f"📊 Баланс: {format_balance(new_balance)}❄️\n\n"
                complete_text += "⏳ Теперь нужно отдохнуть..."
                
                bot.send_message(message.chat.id, complete_text)
                
                # Устанавливаем отдышку
                cooldown_seconds = max(60, int((total_squares / 10 * 60) * 0.3))  # 30% от времени работы
                cooldown_end_time = current_time + timedelta(seconds=cooldown_seconds)
                
                cursor.execute('UPDATE users SET snow_cooldown_end = ? WHERE user_id = ?',
                             (cooldown_end_time, user_id))
                conn.commit()
                
                # После установки отдышки сразу проверяем ее
                # (продолжаем выполнение кода ниже)
        ...
        if cooldown_data and cooldown_data[0]:
            ...
            # Преобразуем время окончания отдышки
            ...
            if cooldown_end and current_time < cooldown_end:
                ...
                cool_msg = "❄️ ОТДЫХАЕТЕ!\n\n"
                cool_msg += f"⏳ Отдохните еще:\n"
                cool_msg += f"{minutes} минут {seconds} секунд\n\n"
                cool_msg += "⚡ После отдыха можно работать снова"
                
                bot.send_message(message.chat.id, cool_msg)
                conn.close()
                return
            else:
                # Отдышка закончилась - очищаем
                cursor.execute('UPDATE users SET snow_cooldown_end = NULL WHERE user_id = ?', (user_id,))
                conn.commit()
        
        # 3. МОЖНО НАЧИНАТЬ НОВУЮ РАБОТУ
        ...
        # Время работы: 10 квадратов в минуту
        work_seconds = int((squares / 10) * 60)
        work_seconds = max(60, min(work_seconds, 1200))  # 1-20 минут
        ...
        cursor.execute('''
            UPDATE users SET 
                ...
                snow_territory = ?
            WHERE user_id = ?
        ''', (squares, work_end_time, current_time, territory['name'], user_id))
        ...
        message_text += "🔄 Используйте /snow чтобы проверить прогресс\n"
        message_text += "⏳ Работа завершится автоматически!"
        
        bot.send_message(message.chat.id, message_text)
        
        # УДАЛИЛИ сложный таймер - просто сохраняем время окончания
    ...
    try:
        ...
        cursor.execute('''
            SELECT snow_job_end_time, snow_cooldown_end, snow_territory, snow_job_total, 
                   balance, last_snow_work, snow_jobs_count
            FROM users WHERE user_id = ?
        ''', (user_id,))
        ...
        if job_end:
            ...
            if job_end_time:
                if current_time < job_end_time:
                    ...
                    # Устанавливаем отдышку
                    cooldown_seconds = max(60, int((job_total / 10 * 60) * 0.3))
                    cooldown_end_time = current_time + timedelta(seconds=cooldown_seconds)
                    cursor.execute('UPDATE users SET snow_cooldown_end = ? WHERE user_id = ?',
                                 (cooldown_end_time, user_id))
                    ...
                    status_text += "✅ РАБОТА ЗАВЕРШЕНА!\n\n"
                    status_text += f"📍 Место: {territory or '❄️ Зимняя территория'}\n"
                    status_text += f"📐 Очищено: {job_total} м²\n"
                    status_text += f"💰 Заработано: {earnings}❄️\n"
                    status_text += f"📊 Баланс: {format_balance(new_balance)}❄️\n\n"
                    status_text += "⏳ Теперь нужно отдохнуть..."
                    
                    # После этого переходим к проверке отдышки ниже
        
        # 2. Проверяем отдышку (если не было активной работы или она завершилась)
        if cooldown_end:
            ...
            if cooldown_end_time:
                if current_time < cooldown_end_time:
                    ...
                    if "РАБОТА ЗАВЕРШЕНА" not in status_text:
        ...
        # 3. Можно работать (нет активной работы и нет отдышки)
        if "РАБОТА ЗАВЕРШЕНА" not in status_text and "СЕЙЧАС РАБОТАЕТЕ" not in status_text:
            status_text += "✅ ГОТОВ К РАБОТЕ!\n\n"
            status_text += f"📊 Баланс: {format_balance(balance)}❄️\n"
            status_text += f"🛠️ Выполнено работ: {jobs_count}\n\n"
