...
load_house_shop()
# === ЭКО-КУРЬЕР (СТИЛЬНАЯ МИНИМАЛИСТИЧНАЯ ВЕРСИЯ) ===

class EcoCourier:
    """Стильная система доставки с эко-балансом"""
    ...
    def __init__(self):
        ...
        self.cooldown_min = 180  # 3 минуты между сменами
        
    def get_stats(self, user_id):
        """Получить статистику курьера из БД"""
        conn = get_db_connection()
        cursor = conn.cursor()
        
        cursor.execute('''
            SELECT courier_level, courier_xp, courier_deliveries, courier_earned,
                   courier_last_work, courier_cooldown
            FROM users WHERE user_id = ?
        ''', (user_id,))
        ...
        if result:
            level, xp, deliveries, earned, last_work, cooldown = result
            return {
                "level": level or 1,
                "xp": xp or 0,
    ...
    def update_stats(self, user_id, stats):
    ...
    def create_job(self, stats):
        """Создать новую работу"""
        level_data = self.levels[stats["level"]]
        
        # Эко-бонус (зависит от опыта, но не слишком большой)
    ...
    def calculate_pay(self, stats, job):
        """Рассчитать оплату с эко-балансом"""
        level_data = self.levels[stats["level"]]
        base_pay = level_data["pay"]
        
        # Добавляем бонус (максимум +20% к базовой)
        total_pay = base_pay + min(job["bonus"], int(base_pay * 0.2))
        
        # Чем выше уровень, тем меньший процент бонуса (эко-баланс)
        bonus_multiplier = max(0.1, 1 - (stats["level"] * 0.15))
        final_pay = int(base_pay + (job["bonus"] * bonus_multiplier))
        
        return min(final_pay, base_pay + 50)  # Ограничение сверху

eco_courier = EcoCourier()

# === ХРАНЕНИЕ АКТИВНЫХ РАБОТ ===
active_jobs = {}
...
def handle_courier(message):
    ...
    try:
        ...
        # Получаем статистику
        stats = eco_courier.get_stats(user_id)
        
        # Проверяем кулдаун
        current_time = time.time()
        if stats["cooldown"] and current_time < stats["cooldown"]:
...
💡 Следующая смена будет доступна через:
{m} минут {s} секунд
            """
            bot.send_message(message.chat.id, cool_msg)
            return
        
        # Если есть активная работа
        ...
        logging.error(f"Ошибка курьера: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка")

def show_courier_menu(message, user_id, stats):
    """Показать меню курьера"""
    level_data = eco_courier.levels[stats["level"]]
    
    # Следующий уровень
    next_level = min(stats["level"] + 1, 5)
    next_data = eco_courier.levels.get(next_level)
...
▫️ Уровень: {level_data['name']}
▫️ Доставок: {stats['deliveries']}
▫️ Заработано: {format_balance(stats['earned'])}❄️

📊 Прогресс: {stats['xp']}/{level_data['xp_needed']}
{'█' * int(xp_percent/10)}{'░' * (10 - int(xp_percent/10))}

💰 За доставку: {level_data['pay']}❄️
📦 За смену: {level_data['deliveries']} посылок

{get_next_level_info(stats, next_data) if next_data else ''}
    """
...
def get_next_level_info(stats, next_data):
    """Информация о следующем уровне"""
    xp_needed = eco_courier.levels[stats["level"]]["xp_needed"] - stats["xp"]
    
    return f"""
⬆️ До {next_data['name']}:
• Нужно опыта: {xp_needed}
• Доставок: +{next_data['deliveries'] - eco_courier.levels[stats["level"]]["deliveries"]}
• Зарплата: +{next_data['pay'] - eco_courier.levels[stats["level"]]["pay"]}❄️
    """

def show_active_job(message, user_id, stats):
    """Показать активную работу"""
    job = active_jobs[user_id]
    level_data = eco_courier.levels[stats["level"]]
    
    # Прогресс-бар
    progress = (job["done"] / job["total"]) * 10
...
{bar}
{job['done']}/{job['total']} доставок

💰 +{eco_courier.calculate_pay(stats, job)}❄️ за доставку
⚡ Бонус: +{job['bonus']}❄️
    """
    
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton(f"✅ Доставить", callback_data="courier_deliver"),
...
def handle_courier_callback(call):
    ...
    try:
        ...
        if call.data == "courier_start":
            ...
            # Создаем работу
            active_jobs[user_id] = eco_courier.create_job(stats)
            ...
            # Рассчитываем оплату
            pay = eco_courier.calculate_pay(stats, job)
            ...
            if job["done"] >= job["total"]:
                ...
                stats["cooldown"] = current_time + eco_courier.cooldown_min
                
                # Обновляем статистику
                eco_courier.update_stats(user_id, stats)
...
💰 Заработано: {format_balance(total_earnings)}❄️
💳 Баланс: {format_balance(new_balance)}❄️

⏳ Следующая через 3 минуты
                """
                
                if stats["xp"] == 0 and stats["level"] > 1:  # Только что повысили уровень
                    new_level = eco_courier.levels[stats["level"]]
                ...
                # Обновляем работу
                job["address"] = random.choice(eco_courier.addresses)
                job["package"] = random.choice(eco_courier.packages)
                
                # Сохраняем статистику
                eco_courier.update_stats(user_id, stats)
                
                # Показываем обновленную работу
                show_active_job(call.message, user_id, stats)
                bot.answer_callback_query(call.id, f"✅ +{pay}❄️")
            ...
            if user_id in active_jobs:
                ...
                bot.edit_message_text(
                    "❌ Смена отменена\n\n" +
                    (f"💰 Сохранено: {format_balance(job['earnings'])}❄️" if job["earnings"] > 0 else "💸 Ничего не заработано"),
                    call.message.chat.id,
                    call.message.message_id,
                    parse_mode='Markdown'
                )
        ...
        logging.error(f"Ошибка callback курьера: {e}")
        bot.answer_callback_query(call.id, "❌ Ошибка")

# === АДМИН КОМАНДЫ ===

@bot.message_handler(func=lambda message: message.text.lower().startswith('курьерсброс') and is_admin(message.from_user.id))
def reset_courier_stats(message):
    """Сброс статистики курьера (админ)"""
    try:
        target = message.text.split()[1] if len(message.text.split()) > 1 else None
        
        if not target:
            bot.send_message(message.chat.id, "❌ Укажите ID или @username")
            return
        
        user_id = get_user_id_from_input(target)
        if not user_id:
            bot.send_message(message.chat.id, "❌ Пользователь не найден")
            return
        ...
        cursor.execute('''
            UPDATE users SET
                ...
                courier_earned = 0,
                courier_last_work = NULL,
                courier_cooldown = NULL
            WHERE user_id = ?
        ''', (user_id,))
        ...
        bot.send_message(message.chat.id, f"✅ Статистика курьера сброшена")
        
    except Exception as e:
        logging.error(f"Ошибка сброса курьера: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка")

# === ИНИЦИАЛИЗАЦИЯ БАЗЫ ===

def init_courier_db():
    """Инициализация таблицы для курьера"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        courier_columns = [
            ("courier_level", "INTEGER DEFAULT 1"),
            ("courier_xp", "INTEGER DEFAULT 0"),
            ("courier_deliveries", "INTEGER DEFAULT 0"),
            ("courier_earned", "INTEGER DEFAULT 0"),
            ("courier_last_work", "INTEGER"),
            ("courier_cooldown", "INTEGER")
        ]
        
        for col_name, col_type in courier_columns:
        ...
        logging.info("База курьера инициализирована")
        
    except Exception as e:
        logging.error(f"Ошибка инициализации курьера: {e}")
...
def cleanup_old_jobs():
    ...
    while True:
        ...
        for user_id, job in active_jobs.items():
            # Если работа старше 30 минут
            if current_time - job.get("started", current_time) > 1800:
                to_remove.append(user_id)
        
        for user_id in to_remove:
            # Сохраняем прогресс перед удалением
            if user_id in active_jobs:
                job = active_jobs[user_id]
                stats = eco_courier.get_stats(user_id)
        ...
        if to_remove:
            logging.info(f"Очищено {len(to_remove)} старых работ")
...
<b>🔢 Кол-во:</b> <b>{max_activations}</b>
        ...
        logging.info(f"Админ {message.from_user.id} создал чеф: {code} на {amount}❄️ × {max_activations}")
        ...
        if last_activation and last_activation[0]:
            ...
            if time_diff.total_seconds() < 1800:  # 1800 секунд = 30 минут
                ...
                bot.send_message(user_id,
                    f"⏳ *Кулдаун на активацию чеков!*\n\n"
                    f"Вы недавно активировали чек.\n"
                    f"Следующая активация через:\n"
