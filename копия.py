...
def init_db():
    ...
    try:
        ...
        # Таблица пользователей (полная структура)
        cursor.execute('''
        ...
        # Добавляем last_bonus если его нет
        if 'last_bonus' not in existing_columns:
            cursor.execute("ALTER TABLE users ADD COLUMN last_bonus INTEGER DEFAULT 0")
            logging.info("✅ Добавлена колонка last_bonus")
        
        # Добавляем снежные колонки если их нет
        snow_columns = ['last_snow_work', 'snow_cooldown_end', 'current_snow_job', 
                       'snow_job_progress', 'snow_job_total', 'snow_job_end_time', 'snow_territory']
        
        for column in snow_columns:
            if column not in existing_columns:
                if column in ['last_snow_work', 'snow_cooldown_end', 'snow_job_end_time']:
                    cursor.execute(f"ALTER TABLE users ADD COLUMN {column} TIMESTAMP")
                elif column in ['snow_job_progress', 'snow_job_total']:
        ...
        # Проверяем колонки в таблице checks
        cursor.execute("PRAGMA table_info(checks)")
        check_columns = [col[1] for col in cursor.fetchall()]
        
        if 'created_at' not in check_columns:
            cursor.execute("ALTER TABLE checks ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP")
            logging.info("✅ Добавлена колонка created_at в checks")
...
# === ОБНОВЛЕННАЯ ФУНКЦИЯ ПОЛУЧЕНИЯ ТОПА БАЛАНСОВ ===
def get_balance_top_page(page=1, limit=5):
    """Получает топ пользователей по балансу с никнеймами"""
    offset = (page - 1) * limit
...
# === ОБНОВЛЕННАЯ ФУНКЦИЯ ПОЛУЧЕНИЯ ТОПА СКАМА ===
...
def create_top_message(user_id, page=1):
    """Создает сообщение с топом пользователей с никнеймами"""
    try:
        ...
        has_nickname = top_data.get('has_nickname', False)
        ...
        if not top_users:
            ...
            for i, user in enumerate(top_users):
                if has_nickname:
                    user_id_db, nickname, username, first_name, value, position = user
                    display_name = get_user_display_name(user_id_db, username, first_name, nickname)
                else:
                    user_id_db, _, username, first_name, value, position = user
                    display_name = get_user_display_name(user_id_db, username, first_name)
                ...
                # Добавляем ссылку если это юзернейм
                if display_name.startswith('@'):
                    display_html = f'<a href="https://t.me/{display_name[1:]}">{display_name}</a>'
                else:
        ...
        if user_position:
            if mode == 'balance':
                ...
                cursor.execute("PRAGMA table_info(users)")
                columns = [col[1] for col in cursor.fetchall()]
                has_nickname_col = 'nickname' in columns
                
                if has_nickname_col:
                    cursor.execute('SELECT balance, nickname FROM users WHERE user_id = ?', (user_id,))
                else:
                    cursor.execute('SELECT balance, NULL FROM users WHERE user_id = ?', (user_id,))
                    
                user_data = cursor.fetchone()
                conn.close()
                
                if user_data:
                    balance = user_data[0] if user_data[0] is not None else 0
                    nickname = user_data[1] if has_nickname_col else None
                    
                    # Получаем отображаемое имя
                    user_info = bot.get_chat(user_id)
                    display_name = get_user_display_name(user_id, user_info.username, user_info.first_name, nickname)
                ...
                if has_nickname_col:
                    ...
                    nickname = nickname_result[0] if nickname_result else None
                else:
                    nickname = None
...
def handle_games_button(message):
    """Обработчик кнопки "Игры" в меню"""
    try:
        if is_spam(message.from_user.id):
            return
        ...
        show_games_menu(message.chat.id)
    
    except Exception as e:
        logging.error(f"Ошибка в handle_games_button: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка загрузки игр. Попробуйте снова.")

# === ОБРАБОТЧИК КОМАНДЫ /game ===
@bot.message_handler(commands=['game', 'games', 'игры', 'игра'])
def handle_game_command(message):
    """Обработчик команд /game, /games, /игры, /игра"""
    try:
        if is_spam(message.from_user.id):
            return
        ...
        # Показываем что команда обработана
        bot.send_chat_action(message.chat.id, 'typing')
        
        show_games_menu(message.chat.id)
    
    except Exception as e:
        logging.error(f"Ошибка в handle_game_command: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка загрузки игр. Попробуйте снова.")

# === ОБЩАЯ ФУНКЦИЯ ДЛЯ ПОКАЗА МЕНЮ ИГР ===
def show_games_menu(chat_id):
    """Показывает меню игр (общая функция для всех обработчиков)"""
    markup = InlineKeyboardMarkup(row_width=3)
    
    markup.add(
        InlineKeyboardButton("🎰 Слоты", callback_data="casino_slots"),
        InlineKeyboardButton("🎯 Рулетка", callback_data="casino_roulette"),
        InlineKeyboardButton("🎲 Кубик", callback_data="casino_dice"),
        InlineKeyboardButton("💥 Краш", callback_data="casino_crash"),
        InlineKeyboardButton("🏀 Баскетбол", callback_data="casino_basket"),
        InlineKeyboardButton("⚽ Футбол", callback_data="casino_football"),
        InlineKeyboardButton("🎯 Дартс", callback_data="casino_darts"),
        InlineKeyboardButton("🎳 Боулинг", callback_data="casino_bowling"),
        InlineKeyboardButton("🪙 Монетка", callback_data="casino_coin")
    )
    
    # Добавляем кнопку информации о командах
    markup.row(InlineKeyboardButton("📋 Команды игр", callback_data="casino_commands"))
    
    casino_mini_text = """
🎰 *МИНИМАЛИСТИЧНОЕ КАЗИНО*

*Выберите игру через кнопки ниже*
или используйте команды:

*🎰 Слоты:* `слот [ставка]`
*🎯 Рулетка:* `рул [ставка]`
*🎲 Кубик:* `куб [ставка]`
*💥 Краш:* `краш [множитель] [ставка]`
*🏀 Баскетбол:* `бск [ставка]`
*⚽ Футбол:* `фтб [ставка]`
*🎯 Дартс:* `дартс [ставка]`
*🎳 Боулинг:* `боул [ставка]`
*🪙 Монетка:* `орёл [ставка]` или `решка [ставка]`

*💰 Примеры ставок:*
`1000` `1к` `10к` `100к` `1кк` `1ккк`
    """
    
    bot.send_message(chat_id, casino_mini_text, reply_markup=markup, parse_mode='Markdown')

# === ОБРАБОТЧИК КОЛБЭКОВ ДЛЯ ИНФОРМАЦИИ ОБ ИГРАХ ===
@bot.callback_query_handler(func=lambda call: call.data.startswith('casino_'))
def casino_callback_handler(call):
    """Обработка колбэков информации об играх"""
    try:
        if is_spam(call.from_user.id):
            bot.answer_callback_query(call.id, "⏳ Слишком быстро! Подождите немного.")
            return
        ...
        game_type = call.data.replace('casino_', '')
...
*Команда:* `рул [тип] [ставка]`
*Примеры:* 
`рул красный 1000к`
`рул 17 500к`
`рул чет 1кк`
*Шанс:* 48.6% (красный/черный)
            """,
...
*Команда:* `краш [множитель] [ставка]`
*Пример:* `краш 2.0 1000к`
*Мин. ставка:* 50к❄️
*Время игры:* 3 секунды
            """,
...
*Команда:* `орёл [ставка]` или `решка [ставка]`
*Пример:* `орёл 1000`
*Шанс:* 50%
            """,
...
*🎰 Слоты:* `слот [ставка]`
*🎯 Рулетка:* `рул [тип] [ставка]`
*🎲 Кубик:* `куб [тип] [ставка]`
*💥 Краш:* `краш [множитель] [ставка]`
*🏀 Баскетбол:* `бск [ставка]`
*⚽ Футбол:* `фтб [ставка]`
*🎯 Дартс:* `дартс [ставка]`
...
`все` - вся сумма
        ...
        if game_type in game_info:
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton("⬅️ Назад в меню", callback_data="casino_back"))
            ...
            # Возвращаем к основному меню игр
            show_games_menu(call.message.chat.id)
        
        bot.answer_callback_query(call.id)
        
    except Exception as e:
        logging.error(f"Ошибка в casino_callback_handler: {e}")
        bot.answer_callback_query(call.id, "❌ Ошибка")
