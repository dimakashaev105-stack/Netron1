...
MAX_BONUS = 3000  # Максимальный бонус
...
def handle_daily_bonus(message):
    try:
        ...
        bonus_text = f"🎁 *Бонус*\n\n"
        bonus_text += f"❄️ *{MIN_BONUS}-{MAX_BONUS}❄️*\n"
...
load_house_shop()
# === СИСТЕМА РАБОТЫ КУРЬЕРА ===

# Глобальные переменные для курьера
COURIER_JOBS = {}  # {user_id: job_data}
COURIER_COOLDOWN = {}  # {user_id: end_time}
COURIER_STATS = {}  # {user_id: {level, deliveries, total_earned}}

# Настройки системы курьера
COURIER_LEVELS = {
    1: {"name": "🛵 Начинающий курьер", "deliveries_per_job": 1, "base_pay": 100, "xp_to_next": 10, "cooldown": 120},
    2: {"name": "🚲 Опытный курьер", "deliveries_per_job": 2, "base_pay": 200, "xp_to_next": 25, "cooldown": 120},
    3: {"name": "🚗 Профессиональный курьер", "deliveries_per_job": 3, "base_pay": 400, "xp_to_next": 50, "cooldown": 120},
    4: {"name": "🚚 Мастер доставки", "deliveries_per_job": 4, "base_pay": 700, "xp_to_next": 100, "cooldown": 120},
    5: {"name": "✈️ Легенда доставки", "deliveries_per_job": 5, "base_pay": 1000, "xp_to_next": 200, "cooldown": 120},
}
...
def handle_courier(message):
    try:
        ...
        # Проверяем кулдаун
        if user_id in COURIER_COOLDOWN:
            cooldown_end = COURIER_COOLDOWN[user_id]
            if current_time < cooldown_end:
                time_left = int(cooldown_end - current_time)
                minutes = time_left // 60
                seconds = time_left % 60
                bot.send_message(message.chat.id, f"⏳ Отдых: {minutes}м {seconds}с")
                return
        ...
        # Если нет активной работы, показываем меню
        if user_id not in COURIER_JOBS:
            show_courier_menu(message, user_id, stats, level_data)
        else:
            # Если есть активная работа, показываем её
            job = COURIER_JOBS[user_id]
            show_courier_job(message, user_id, job, stats, level_data)
            
    except Exception as e:
        logging.error(f"Ошибка в курьере: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка загрузки работы курьера")
...
def save_courier_stats(user_id, stats):
    """Сохранить статистику курьера"""
    COURIER_STATS[user_id] = stats

def show_courier_menu(message, user_id, stats, level_data):
    """Показать меню курьера"""
    next_level = stats["level"] + 1
    next_level_data = COURIER_LEVELS.get(next_level)
    
    message_text = f"🚚 *РАБОТА КУРЬЕРА*\n\n"
    message_text += f"📊 *Ваш уровень:* {level_data['name']}\n"
    message_text += f"📦 *Доставок за смену:* {level_data['deliveries_per_job']}\n"
    message_text += f"💰 *Зарплата за доставку:* {level_data['base_pay']}❄️\n"
    message_text += f"🎯 *Доставлено всего:* {stats['deliveries']}\n"
    message_text += f"💵 *Заработано всего:* {format_balance(stats['total_earned'])}❄️\n"
    message_text += f"📈 *Опыт:* {stats['xp']}/{level_data['xp_to_next']}\n\n"
    
    if next_level_data:
        message_text += f"⬆️ *Следующий уровень:* {next_level_data['name']}\n"
        message_text += f"📦 +{next_level_data['deliveries_per_job']} посылок за смену\n"
        message_text += f"💰 +{next_level_data['base_pay']}❄️ за доставку\n"
        message_text += f"🎯 Нужно опыта: {level_data['xp_to_next'] - stats['xp']}\n\n"
    
    message_text += f"💡 Нажмите '📦 Начать смену' чтобы начать работу"
...
def show_courier_job(message, user_id, job, stats, level_data):
    """Показать текущую работу курьера"""
    deliveries_left = level_data['deliveries_per_job'] - job['completed_deliveries']
    
    message_text = f"🚚 *ДОСТАВКА ПОСЫЛОК*\n\n"
    message_text += f"📊 *Уровень:* {level_data['name']}\n"
    message_text += f"📦 *Осталось доставить:* {deliveries_left}/{level_data['deliveries_per_job']}\n"
    message_text += f"📍 *Текущий адрес:* {job['current_address']}\n"
    message_text += f"📦 *Тип посылки:* {job['current_package']}\n"
    message_text += f"💰 *Оплата за доставку:* {level_data['base_pay']}❄️\n\n"
    message_text += f"⚡ *Бонус за скорость:* +{job['speed_bonus']}❄️\n\n"
    
    # Прогресс-бар
    progress = int((job['completed_deliveries'] / level_data['deliveries_per_job']) * 10)
    progress_bar = "🟩" * progress + "⬜" * (10 - progress)
    message_text += f"{progress_bar}\n\n"
    
    message_text += f"✅ Нажмите 'Доставить' когда окажетесь на месте"
    
    markup = create_courier_delivery_keyboard(job['speed_bonus'])
    bot.send_message(message.chat.id, message_text, parse_mode='Markdown', reply_markup=markup)

def create_courier_delivery_keyboard(speed_bonus):
    """Создать клавиатуру для доставки"""
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton(f"✅ Доставить (+{speed_bonus}❄️)", callback_data="courier_deliver"),
        InlineKeyboardButton("🚫 Отменить смену", callback_data="courier_cancel")
    )
    return markup

def generate_courier_job(user_id):
    """Сгенерировать новую работу курьера"""
    stats = get_courier_stats(user_id)
    level_data = COURIER_LEVELS.get(stats["level"], COURIER_LEVELS[1])
    
    # Случайные адреса для доставки
    addresses = [
        "Ул. Снежная, 15", "Пр. Морозный, 42", "Ул. Ледяная, 7", "Пр. Зимний, 23",
        "Ул. Северная, 56", "Пр. Холодный, 18", "Ул. Белая, 31", "Пр. Ледниковый, 9",
        "Ул. Метельская, 64", "Пр. Снеговой, 27"
    ]
    
    # Типы посылок
    packages = [
        "📦 Коробка с подарками", "🎁 Подарочный набор", "📮 Срочная почта",
        "🍕 Пицца", "☕ Кофе с собой", "📚 Книги", "💻 Ноутбук",
        "👕 Одежда", "🎮 Игровая консоль", "💐 Букет цветов"
    ]
    
    # Случайный бонус за скорость (5-20% от базовой оплаты)
    base_pay = level_data['base_pay']
    speed_bonus = random.randint(int(base_pay * 0.05), int(base_pay * 0.20))
    
    return {
        "start_time": time.time(),
        "completed_deliveries": 0,
        "current_address": random.choice(addresses),
        "current_package": random.choice(packages),
        ...
        if call.data == "courier_start":
            ...
            if user_id in COURIER_COOLDOWN:
                ...
                if current_time < cooldown_end:
                    ...
                    minutes = time_left // 60
                    seconds = time_left % 60
                    bot.answer_callback_query(call.id, f"⏳ Отдых: {minutes}м {seconds}с")
                    return
            
            # Создаем новую работу
            COURIER_JOBS[user_id] = generate_courier_job(user_id)
            
            stats = get_courier_stats(user_id)
            level_data = COURIER_LEVELS.get(stats["level"], COURIER_LEVELS[1])
            
            show_courier_job(call.message, user_id, COURIER_JOBS[user_id], stats, level_data)
            ...
            bot.answer_callback_query(call.id, "🚚 Начинаем смену!")
            ...
            if user_id in COURIER_JOBS:
                ...
                show_courier_menu(call.message, user_id, stats, level_data)
            ...
            # Рассчитываем оплату
            base_pay = level_data['base_pay']
            speed_bonus = job['speed_bonus']
            total_pay = base_pay + speed_bonus
            
            # Добавляем доставку
            job['completed_deliveries'] += 1
            job['total_earnings'] += total_pay
            
            # Обновляем статистику
            stats['deliveries'] += 1
            stats['total_earned'] += total_pay
            stats['xp'] += 1  # 1 XP за доставку
            
            # Проверяем уровень
            if stats['xp'] >= level_data['xp_to_next'] and stats['level'] < len(COURIER_LEVELS):
                # Повышаем уровень
                stats['level'] += 1
                stats['xp'] = 0
                level_up = True
            ...
            # Проверяем, завершена ли смена
            if job['completed_deliveries'] >= level_data['deliveries_per_job']:
                # Завершаем смену
                total_earnings = job['total_earnings']
                update_balance(user_id, total_earnings)
                new_balance = get_balance(user_id)
                
                # Устанавливаем кулдаун (2 минуты)
                COURIER_COOLDOWN[user_id] = time.time() + level_data['cooldown']
                
                del COURIER_JOBS[user_id]
                
                # Сообщение о завершении
                message_text = f"✅ *СМЕНА ЗАВЕРШЕНА!*\n\n"
                message_text += f"📦 *Доставок выполнено:* {level_data['deliveries_per_job']}\n"
                message_text += f"💰 *Заработано:* {format_balance(total_earnings)}❄️\n"
                
                if level_up:
                    new_level_data = COURIER_LEVELS.get(stats['level'])
                    message_text += f"\n🎉 *НОВЫЙ УРОВЕНЬ!*\n"
                    message_text += f"⬆️ *{new_level_data['name']}*\n"
                    message_text += f"📦 +{new_level_data['deliveries_per_job']} посылок за смену\n"
                    message_text += f"💰 +{new_level_data['base_pay']}❄️ за доставку\n"
                
                message_text += f"\n📊 *Всего доставок:* {stats['deliveries']}\n"
                message_text += f"💵 *Всего заработано:* {format_balance(stats['total_earned'])}❄️\n"
                message_text += f"📈 *Текущий опыт:* {stats['xp']}/{COURIER_LEVELS.get(stats['level'], COURIER_LEVELS[1])['xp_to_next']}\n\n"
                message_text += f"⏳ *Следующая смена через 2 минуты*"
                ...
                # Генерируем следующую доставку
                job['current_address'] = random.choice([
                    "Ул. Снежная, 15", "Пр. Морозный, 42", "Ул. Ледяная, 7", "Пр. Зимний, 23",
                    "Ул. Северная, 56", "Пр. Холодный, 18", "Ул. Белая, 31", "Пр. Ледниковый, 9"
                ])
                job['current_package'] = random.choice([
                    "📦 Коробка с подарками", "🎁 Подарочный набор", "📮 Срочная почта",
                    "🍕 Пицца", "☕ Кофе с собой", "📚 Книги", "💻 Ноутбук"
                ])
                # Новый бонус за скорость
                speed_bonus = random.randint(int(base_pay * 0.05), int(base_pay * 0.20))
                job['speed_bonus'] = speed_bonus
                
                show_courier_job(call.message, user_id, job, stats, level_data)
            
            bot.answer_callback_query(call.id, f"✅ Доставлено! +{total_pay}❄️")
            
        elif call.data == "courier_cancel":
            if user_id in COURIER_JOBS:
                # Сохраняем заработанное перед отменой
                job = COURIER_JOBS[user_id]
                if job['total_earnings'] > 0:
                    update_balance(user_id, job['total_earnings'])
                
                # Обновляем статистику
                stats = get_courier_stats(user_id)
                stats['deliveries'] += job['completed_deliveries']
                stats['total_earned'] += job['total_earnings']
                save_courier_stats(user_id, stats)
                ...
                bot.edit_message_text(
                    f"🚫 *СМЕНА ОТМЕНЕНА*\n\n"
                    f"📦 *Доставок выполнено:* {job['completed_deliveries']}\n"
                    f"💰 *Заработано:* {format_balance(job['total_earnings'])}❄️\n\n"
                    f"💡 Можно начать новую смену",
                    call.message.chat.id,
                    call.message.message_id,
                    parse_mode='Markdown'
                )
                
            bot.answer_callback_query(call.id, "🚫 Смена отменена")
            
    except Exception as e:
        logging.error(f"Ошибка в callback курьера: {e}")
        bot.answer_callback_query(call.id, "❌ Ошибка")

# === ОЧИСТКА ДАННЫХ КУРЬЕРА ===
def cleanup_courier_data():
    """Очистка старых данных курьера"""
    while True:
        time.sleep(60)
        current_time = time.time()
        
        # Удаляем старые работы (старше 1 часа)
        jobs_to_remove = []
        ...
        # Удаляем старые кулдауны (старше 1 дня)
        cooldown_to_remove = []
        for user_id, cooldown_end in COURIER_COOLDOWN.items():
            if current_time > cooldown_end + 86400:
                cooldown_to_remove.append(user_id)
        
        for user_id in cooldown_to_remove:
            del COURIER_COOLDOWN[user_id]
...
# === КОМАНДА ДЛЯ СБРОСА СТАТИСТИКИ КУРЬЕРА (админ) ===
...
def handle_reset_courier(message):
    try:
        ...
        logging.error(f"Ошибка сброса статистики курьера: {e}")
        bot.send_message(message.chat.id, f"❌ Ошибка: {str(e)[:100]}")

# === КОМАНДА ДЛЯ ПРОВЕРКИ СТАТИСТИКИ КУРЬЕРА ===
@bot.message_handler(func=lambda message: message.text.lower().startswith('курьерстат'))
def handle_courier_stats_cmd(message):
    try:
        user_id = message.from_user.id
        ...
        message_text = f"📊 *СТАТИСТИКА КУРЬЕРА*\n\n"
        message_text += f"🎯 *Уровень:* {level_data['name']}\n"
        message_text += f"📦 *Всего доставок:* {stats['deliveries']}\n"
        message_text += f"💰 *Всего заработано:* {format_balance(stats['total_earned'])}❄️\n"
        message_text += f"📈 *Опыт:* {stats['xp']}/{level_data['xp_to_next']}\n\n"
        
        # Следующий уровень
        next_level = stats["level"] + 1
        if next_level in COURIER_LEVELS:
            next_data = COURIER_LEVELS[next_level]
            xp_needed = level_data['xp_to_next'] - stats['xp']
            message_text += f"⬆️ *До следующего уровня:* {xp_needed} опыта\n"
            message_text += f"📦 *Будет доставок за смену:* {next_data['deliveries_per_job']}\n"
            message_text += f"💰 *Будет зарплата:* {next_data['base_pay']}❄️\n\n"
        
        # Активная работа
        if user_id in COURIER_JOBS:
            job = COURIER_JOBS[user_id]
            deliveries_left = level_data['deliveries_per_job'] - job['completed_deliveries']
            message_text += f"🚚 *Активная смена:*\n"
            message_text += f"📦 *Осталось доставок:* {deliveries_left}\n"
            message_text += f"💰 *Заработано за смену:* {format_balance(job['total_earnings'])}❄️\n\n"
        ...
        if user_id in COURIER_COOLDOWN:
            ...
            if current_time < cooldown_end:
                ...
                minutes = time_left // 60
                seconds = time_left % 60
                message_text += f"⏳ *Отдых до следующей смены:* {minutes}м {seconds}с"
        
        bot.send_message(message.chat.id, message_text, parse_mode='Markdown')
        
    except Exception as e:
        logging.error(f"Ошибка статистики курьера: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка")
...
def handle_bowling(message):
    try:
        ...
        if result == 6:
            ...
            balance_text = f"💰 Баланс: {format_balance(new_balance)}❄️"
            full_message = f"<blockquote>{result_text}\n<b>{balance_text}</b></blockquote>"
