...
def start(message):
    try:
        ...
        # Обработка реферальных ссылок и чеков
        referred_by = None
        if len(message.text.split()) > 1:
            ...
            if check_data:
                ...
                if ref_user and ref_user[0] != user_id:
                    ...
                    if not current_ref or current_ref[0] is None:
        ...
        bot.send_message(message.chat.id, "✅ Регистрация успешна! Добро пожаловать!", reply_markup=markup)
...
def check_captcha_answer(message):
    try:
        ...
        if user_answer == correct_answer:
            # Сначала убедимся, что пользователь создан
