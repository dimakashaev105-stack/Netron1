...
def check_subscription(user_id):
    try:
        ...
        return channel_member.status in ['member', 'administrator', 'creator']
    except Exception as e:
        logging.error(f"Ошибка проверки подписки для {user_id}: {e}")
        return False

# Обработчик ежедневного бонуса (каждые 30 минут)
    ...
    try:
        ...
        try:
            ...
            if 'last_bonus' not in columns:
                ...
                cursor.execute("ALTER TABLE users ADD COLUMN last_bonus INTEGER")
                conn.commit()
            ...
            logging.error(f"Ошибка при проверке времени бонуса: {e}")
            bot.send_message(message.chat.id, "❌ Ошибка проверки времени")
            return
        finally:
            if conn:
                conn.close()
...
def handle_claim_bonus(call):
    ...
    try:
        ...
        # Обновляем время бонуса - сохраняем как timestamp (число)
...
def handle_delete_user(message):
    try:
        ...
        parts = message.text.split()
        if len(parts) < 2:
            bot.send_message(message.chat.id, "❌ Неверный формат. Пример: удалить 5255608302")
            return
        ...
        markup = InlineKeyboardMarkup()
        markup.row(
            InlineKeyboardButton("✅ ДА, УДАЛИТЬ ВСЁ", callback_data=f"delete_confirm_{target_user_id}"),
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT username, first_name, balance, bank_deposit FROM users WHERE user_id = ?', (target_user_id,))
        user_data = cursor.fetchone()
        conn.close()
        ...
        username, first_name, balance, bank_deposit = user_data
        
        confirm_msg = f"⚠️ ВНИМАНИЕ: ОПАСНАЯ КОМАНДА!\n\n"
        confirm_msg += f"Вы собираетесь ПОЛНОСТЬЮ удалить пользователя:\n"
        confirm_msg += f"👤 ID: {target_user_id}\n"
        confirm_msg += f"📛 Имя: {first_name}\n"
        confirm_msg += f"🔗 Юзернейм: @{username if username else 'нет'}\n"
        confirm_msg += f"💰 Баланс: {format_balance(balance)}❄️\n"
        confirm_msg += f"🏦 В банке: {format_balance(bank_deposit)}❄️\n\n"
        confirm_msg += f"❌ Будет удалено:\n"
        confirm_msg += f"• Все данные пользователя\n"
        confirm_msg += f"• Все чеки созданные им\n"
        confirm_msg += f"• Все активации чеков\n\n"
        confirm_msg += f"⚠️ Это действие НЕОБРАТИМО!"
        
        bot.send_message(message.chat.id, confirm_msg, reply_markup=markup)
    
    except Exception as e:
        logging.error(f"Ошибка в handle_delete_user: {e}")
...
def delete_callback_handler(call):
    try:
        ...
        if call.data.startswith('delete_confirm_'):
            ...
            # 1. Сохраняем информацию для отчета (перед удалением)
            cursor.execute('SELECT username, first_name, balance, bank_deposit, video_cards FROM users WHERE user_id = ?', (target_user_id,))
            ...
            username, first_name, balance, bank_deposit, video_cards = user_info
            
            # 2. Удаляем ВСЕ чеки созданные этим пользователем
            cursor.execute('DELETE FROM checks WHERE created_by = ?', (target_user_id,))
            checks_deleted = cursor.rowcount
            
            # 3. Удаляем ВСЕ активации чеков этим пользователем
            cursor.execute('DELETE FROM check_activations WHERE user_id = ?', (target_user_id,))
            activations_deleted = cursor.rowcount
            ...
            # 5. Формируем отчет об удалении
            result_text = f"✅ ПОЛЬЗОВАТЕЛЬ ПОЛНОСТЬЮ УДАЛЕН!\n\n"
            result_text += f"📋 Отчет об удалении:\n"
            result_text += f"👤 Пользователь: {first_name} (@{username if username else 'нет'})\n"
            result_text += f"🆔 ID: {target_user_id}\n"
            result_text += f"💰 Удалено средств: {format_balance(balance)}❄️ (баланс)\n"
            result_text += f"🏦 Удалено в банке: {format_balance(bank_deposit)}❄️\n"
            result_text += f"🖥 Удалено видеокарт: {video_cards}\n"
            result_text += f"💳 Удалено чеков: {checks_deleted}\n"
            result_text += f"🎫 Удалено активаций: {activations_deleted}\n\n"
            result_text += f"⚠️ Все данные пользователя БЕЗВОЗВРАТНО удалены!"
            ...
            # 6. Пытаемся уведомить удаленного пользователя
            try:
                bot.send_message(
                    target_user_id,
                    f"🚫 ВАШ АККАУНТ БЫЛ УДАЛЕН АДМИНИСТРАТОРОМ!\n\n"
                    f"👮 Администратор: @{call.from_user.username if call.from_user.username else 'Неизвестно'}\n"
                    f"⏰ Время удаления: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                    f"❌ Все ваши данные удалены:\n"
                    f"• Баланс: {format_balance(balance)}❄️\n"
                    f"• Вклады: {format_balance(bank_deposit)}❄️\n"
                    f"• Чеки и активации\n\n"
                    f"⚠️ Для регистрации заново используйте /start"
                )
            except:
        ...
        logging.error(f"Ошибка в delete_callback_handler: {e}")
