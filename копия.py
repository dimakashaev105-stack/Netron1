...
def init_db():
    ...
    try:
        ...
        # Таблица вызовов в кости (добавлена в правильном месте)
        cursor.execute('''
...
def start(message):
    try:
        ...
        if not user_data:
            ...
            if referred_by:
                REFERRAL_BONUS = 250  # Бонус за реферала
...
def handle_scam(message):
    try:
        ...
        if result:
            ...
            REFERRAL_BONUS = 250  # Бонус за каждого реферала
...
def handle_dice_bet(message):
    try:
        ...
        if not message.reply_to_message:
            bot.send_message(message.chat.id, "🎲 Для игры в кости ответьте на сообщение пользователя")
            return
        ...
        # Проверяем бан оппонента
        target_banned, target_reason = is_banned(target_user_id)
        if target_banned:
            bot.send_message(message.chat.id, "❌ Оппонент забанен в боте")
            return
        
        parts = message.text.lower().split()
        if len(parts) < 2:
            bot.send_message(message.chat.id, "❌ Формат: кости [ставка]\nПример: кости 1000к")
            return
        ...
        if user_balance < bet_amount:
            bot.send_message(message.chat.id, 
                           ...
                           f"Нужно: ❄️{format_balance(bet_amount)}\n"
                           f"У вас: ❄️{format_balance(user_balance)}")
            return
        ...
        if target_balance < bet_amount:
            bot.send_message(message.chat.id, 
                           f"❌ У оппонента недостаточно средств!\n"
                           f"Нужно: ❄️{format_balance(bet_amount)}\n"
                           f"У него: ❄️{format_balance(target_balance)}")
            return
        ...
        user_name = f"@{user_data[1]}" if user_data and user_data[1] else (user_data[0] if user_data else "Игрок")
        target_name = f"@{target_data[1]}" if target_data and target_data[1] else (target_data[0] if target_data else "Игрок")
        
        # Создаем вызов
        markup = InlineKeyboardMarkup()
        markup.row(
            InlineKeyboardButton("✅ Принять", callback_data=f"dice_accept_{user_id}_{target_user_id}_{bet_amount}"),
            InlineKeyboardButton("❌ Отказаться", callback_data=f"dice_decline_{user_id}_{target_user_id}_{bet_amount}")
        )
        
        challenge_text = f"🎲 <b>ВЫЗОВ НА КОСТИ</b> 🎲\n\n"
        challenge_text += f"👤 <b>{user_name}</b> вызывает {target_name}!\n"
        challenge_text += f"💰 <b>Ставка:</b> ❄️{format_balance(bet_amount)}\n\n"
        challenge_text += f"🎯 <b>Правила:</b>\n"
        challenge_text += f"• Каждый бросает кубик\n"
        challenge_text += f"• У кого больше очков - забирает ставку ×2\n"
        challenge_text += f"• При ничье - возврат ставок"
        
        bot.send_message(message.chat.id, challenge_text, reply_markup=markup, parse_mode='HTML')
        
    except Exception as e:
        logging.error(f"Ошибка в handle_dice_bet: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка создания вызова")

# Обработчик принятия/отклонения вызова
...
def handle_dice_response(call):
    try:
        ...
        # Проверяем, что это тот пользователь
        if action == "accept" and call.from_user.id != target_id:
            bot.answer_callback_query(call.id, "❌ Не ваш вызов!")
            return
        
        if action == "decline" and call.from_user.id != target_id:
            bot.answer_callback_query(call.id, "❌ Не ваш вызов!")
            return
        
        if action == "decline":
            bot.answer_callback_query(call.id, "❌ Вызов отклонен")
            bot.edit_message_text(
                f"❌ Вызов отклонен пользователем",
                call.message.chat.id,
                call.message.message_id
            )
            return
        ...
        if challenger_balance < bet_amount or target_balance < bet_amount:
            bot.edit_message_text(
                f"❌ У одного из игроков недостаточно средств",
                call.message.chat.id,
                call.message.message_id
            )
            return
        ...
        cursor.execute('SELECT first_name, username FROM users WHERE user_id = ?', (challenger_id,))
        challenger_data = cursor.fetchone()
        cursor.execute('SELECT first_name, username FROM users WHERE user_id = ?', (target_id,))
        target_data = cursor.fetchone()
        conn.close()
        
        challenger_name = f"@{challenger_data[1]}" if challenger_data and challenger_data[1] else (challenger_data[0] if challenger_data else "Игрок 1")
        target_name = f"@{target_data[1]}" if target_data and target_data[1] else (target_data[0] if target_data else "Игрок 2")
        
        # Бросаем кубики с задержкой
        time.sleep(1)
        ...
        if challenger_score > target_score:
            ...
            result_emoji = "🎉"
            result_text = f"<b>{winner_name} ВЫИГРАЛ!</b>"
            ...
            # Ничья
            update_balance(challenger_id, bet_amount)  # Возвращаем ставки
            update_balance(target_id, bet_amount)
            result_emoji = "🤝"
        ...
        result_message = f"{result_emoji} <b>РЕЗУЛЬТАТЫ ИГРЫ</b> {result_emoji}\n\n"
        result_message += f"👤 <b>{challenger_name}:</b> {challenger_score} очков\n"
        result_message += f"👤 <b>{target_name}:</b> {target_score} очков\n\n"
        result_message += f"{result_text}\n\n"
        
        if win_amount > 0:
            result_message += f"💰 <b>Выигрыш:</b> ❄️{format_balance(win_amount)}"
        else:
            result_message += f"💰 <b>Ставки возвращены</b>"
        ...
        logging.error(f"Ошибка в handle_dice_response: {e}")
        try:
