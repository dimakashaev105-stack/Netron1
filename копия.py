...
def top_callback_handler(call):
    try:
        ...
        try:
            ...
            pass  # Добавляем pass для пустого except

# === КОМАНДА ДЛЯ БЛОКИРОВКИ РЕФЕРАЛЬНЫХ СВЯЗЕЙ С АДМИНАМИ ===
...
def handle_block_admin_refs(message):
    """Блокирует реферальные связи с администраторами"""
    try:
        if not is_admin(message.from_user.id):
            return
        ...
        for admin_id in admin_ids:
            try:
                # 1. Удаляем рефералов у админа (если кто-то его пригласил)
...
def handle_mining(message):
    try:
        ...
        # Получаем информацию о елках пользователя
        cursor.execute('''
            SELECT video_cards, last_mining_collect, balance, first_name 
            FROM users WHERE user_id = ?
        ''', (user_id,))
        ...
        video_cards, last_mining_collect, balance, first_name = result
        ...
        if last_mining_collect and last_mining_collect > 0 and video_cards > 0:
            ...
            hours_passed = int((current_time - last_mining_collect) / 3600)
            if hours_passed > 0:
                # НОВАЯ ФОРМУЛА: на уровне N - N елок в час
                hourly_income = video_cards
                total_accumulated = hourly_income * hours_passed
        
        # Создаем сообщение
        message_text = f"🎄 *Майнинг елок*\n\n"
        
        if video_cards > 0:
            # Рассчитываем текущий доход в час
            current_hourly = video_cards
            
            message_text += f"✨ *Уровень:* {video_cards}\n"
            message_text += f"💰 *Доход:* {current_hourly} 🎄/час\n"
            ...
            message_text += f"✨ *Уровень:* 0\n"
            message_text += f"💰 *Доход:* 0 🎄/час\n"
            message_text += f"📊 *Накоплено:* 0 🎄\n\n"
        
        # Добавляем информацию о следующем уровне
        next_level = video_cards + 1
        next_level_price = calculate_level_price(next_level)
        
        message_text += f"🆙 *След. уровень:* {next_level}\n"
        message_text += f"💰 *Доход:* {next_level} 🎄/час\n"
        message_text += f"❄️ *Стоимость:* {format_balance(next_level_price)}❄️\n\n"
        
        # Таблица уровней
        message_text += "*📈 Уровни доходов:*\n"
        
        # Показываем несколько уровней
        for level in range(max(1, video_cards - 2), video_cards + 6):
            if level < 1:
                continue
            level_income = level  # N елок в час на уровне N
            level_price = calculate_level_price(level)
            
            level_marker = ""
            if level == video_cards:
                level_marker = "👈"
            elif level == next_level:
                level_marker = "🔜"
            
            message_text += f"🎄 Ур. {level}: {level_income}🎄/час - {format_balance(level_price)}❄️ {level_marker}\n"
        
        message_text += f"\n💡 *Формула:* На уровне N - N елок в час"
        ...
        if video_cards > 0:
            if total_accumulated > 0:
                markup.add(InlineKeyboardButton("💰 Забрать елки", callback_data="collect_elves"))
            else:
                markup.add(InlineKeyboardButton("⏳ Елки копятся...", callback_data="no_elves"))
        
        markup.add(
            InlineKeyboardButton(f"🆙 Улучшить ({format_balance(next_level_price)}❄️)", 
                               callback_data="upgrade_mining"),
            InlineKeyboardButton("🔄 Обновить", callback_data="refresh_mining")
        )
...
def calculate_level_price(level):
    """Рассчитывает стоимость улучшения до уровня"""
    # Базовая формула: 1000 * level^2
    return int(1000 * (level ** 2))

@bot.callback_query_handler(func=lambda call: call.data == "upgrade_mining")
def handle_upgrade_mining(call):
    """Улучшение уровня майнинга"""
    try:
        user_id = call.from_user.id
        ...
        # Получаем текущий уровень и баланс
        ...
        current_level, balance = result
        next_level = current_level + 1
        upgrade_price = calculate_level_price(next_level)
        
        # Проверяем баланс
        if balance < upgrade_price:
            bot.answer_callback_query(
                call.id, 
                f"❌ Недостаточно средств! Нужно: {format_balance(upgrade_price)}❄️",
                show_alert=True
            )
            conn.close()
            return
        ...
        # Списываем деньги
        cursor.execute('UPDATE users SET balance = balance - ?, video_cards = ? WHERE user_id = ?',
                     (upgrade_price, next_level, user_id))
        
        # Сбрасываем время сбора (чтобы не было накоплений со старого уровня)
        ...
        # Обновляем сообщение
        new_balance = balance - upgrade_price
        
        # Создаем новое сообщение
        message_text = f"🎄 *Майнинг елок*\n\n"
        message_text += f"✅ *Уровень повышен до {next_level}!*\n"
        message_text += f"💰 *Новый доход:* {next_level} 🎄/час\n"
        message_text += f"💸 *Потрачено:* {format_balance(upgrade_price)}❄️\n"
        message_text += f"❄️ *Баланс:* {format_balance(new_balance)}❄️\n\n"
        
        # Следующий уровень
        next_next_level = next_level + 1
        next_next_price = calculate_level_price(next_next_level)
        
        message_text += f"🆙 *След. уровень:* {next_next_level}\n"
        message_text += f"💰 *Будет доход:* {next_next_level} 🎄/час\n"
        message_text += f"❄️ *Стоимость:* {format_balance(next_next_price)}❄️"
        
        # Обновляем клавиатуру
        markup = InlineKeyboardMarkup(row_width=2)
        markup.add(
            InlineKeyboardButton(f"🆙 Улучшить ({format_balance(next_next_price)}❄️)", 
                               callback_data="upgrade_mining"),
            InlineKeyboardButton("🔄 Обновить", callback_data="refresh_mining")
        )
        ...
        bot.answer_callback_query(call.id, f"✅ Улучшено до уровня {next_level}!")
        
        conn.close()
        
    except Exception as e:
        logging.error(f"Ошибка улучшения майнинга: {e}")
        bot.answer_callback_query(call.id, "❌ Ошибка")

@bot.callback_query_handler(func=lambda call: call.data == "collect_elves")
def handle_collect_elves(call):
    """Сбор накопленных елок"""
    try:
        user_id = call.from_user.id
        ...
        # Получаем информацию о елках
        cursor.execute('''
            SELECT video_cards, last_mining_collect, balance 
            FROM users WHERE user_id = ?
        ''', (user_id,))
        ...
        video_cards, last_mining_collect, balance = result
        
        if video_cards == 0:
            bot.answer_callback_query(call.id, "❌ Нет уровня майнинга!")
            return
        
        if not last_mining_collect or last_mining_collect <= 0:
            bot.answer_callback_query(call.id, "❌ Нет накоплений")
            return
        ...
        if hours_passed <= 0:
            bot.answer_callback_query(call.id, "❌ Елки еще копятся...")
            return
        
        # Новая формула: video_cards елок в час
        hourly_income = video_cards
        total_accumulated = hourly_income * hours_passed
        
        # Начисляем елки на баланс
        cursor.execute('UPDATE users SET balance = balance + ?, last_mining_collect = ? WHERE user_id = ?',
                     (total_accumulated, int(time.time()), user_id))
        ...
        message_text += f"✅ *Собрано:* {total_accumulated} 🎄\n"
        message_text += f"❄️ *Баланс:* {format_balance(new_balance)}❄️\n\n"
        ...
        bot.answer_callback_query(call.id, f"✅ Собрано {total_accumulated}🎄!")
        
        conn.close()
        
    except Exception as e:
        logging.error(f"Ошибка сбора елок: {e}")
        bot.answer_callback_query(call.id, "❌ Ошибка")

@bot.callback_query_handler(func=lambda call: call.data == "refresh_mining")
def handle_refresh_mining(call):
    """Обновление информации о майнинге"""
    try:
        user_id = call.from_user.id
        ...
        logging.error(f"Ошибка обновления майнинга: {e}")
        bot.answer_callback_query(call.id, "❌ Ошибка")

@bot.callback_query_handler(func=lambda call: call.data == "no_elves")
def handle_no_elves(call):
    """Обработка нажатия когда нет елок"""
    bot.answer_callback_query(call.id, "⏳ Елки еще не накопились...")
