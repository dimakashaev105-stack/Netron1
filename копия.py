...
def handle_transfer(message):
    try:
        ...
        if len(parts) < 3:
            bot.send_message(message.chat.id, 
                           "❌ Формат: передать @username сумма\n"
                           "Пример: передать @user 1000к\n\n"
                           "Или ответьте на сообщение с суммой")
            return
        ...
        # Вариант 1: Ответ на сообщение (без указания юзернейма)
        if target_identifier.startswith('@') and len(parts) == 2:
            # Это формат "передать @username" (без суммы)
            # Проверяем, может сумма указана как отдельный аргумент
            if message.reply_to_message:
                # Если есть ответ, используем ID из ответа
                target_user_id = message.reply_to_message.from_user.id
                amount_text = target_identifier  # Перемещаем аргумент
                target_identifier = f"@{message.reply_to_message.from_user.username}" if message.reply_to_message.from_user.username else message.reply_to_message.from_user.first_name
            else:
                bot.send_message(message.chat.id, "❌ Укажите сумму или ответьте на сообщение")
                return
        
        # Вариант 2: Ответ на сообщение + сумма
        elif message.reply_to_message:
            target_user_id = message.reply_to_message.from_user.id
            target_identifier = f"@{message.reply_to_message.from_user.username}" if message.reply_to_message.from_user.username else message.reply_to_message.from_user.first_name
        
        # Вариант 3: Передача по юзернейму
            ...
            if result:
                ...
                conn.close()
                bot.send_message(message.chat.id, f"❌ Пользователь @{username} не найден или забанен")
                return
            
            conn.close()
            
        # Вариант 4: Передача по ID
        else:
            try:
        ...
        if not target_data:
            # Если пользователя нет в базе, пытаемся получить информацию из Telegram
            try:
                ...
                bot.send_message(message.chat.id, "❌ Не удалось найти получателя")
                conn.close()
                return
        else:
        ...
        # Логируем передачу
        logging.info(f"Передача: {user_id} -> {target_user_id} сумма {transfer_amount}")
    
    except Exception as e:
        logging.error(f"Ошибка в передаче денег: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка при передаче. Попробуйте позже.")

# Дополнительная команда для информации
@bot.message_handler(func=lambda message: message.text.lower() == 'перевод')
def handle_transfer_info(message):
    info_text = (
        "💸 ПЕРЕВОД ДЕНЕГ\n\n"
        "📋 Форматы команд:\n"
        "• `передать @username сумма`\n"
        "• `кинуть 123456789 1000к` (по ID)\n"
        "• Ответьте `передать сумма` на сообщение\n\n"
        "📝 Примеры:\n"
        "`передать @user 1000к`\n"
        "`кинуть 123456789 1ккк`\n"
        "`дать @user 500`\n\n"
        "⚠️ Минимум: 10❄️\n"
        "❌ Нельзя переводить себе\n"
        "🚫 Забаненным переводить нельзя"
    )
