...
def init_db():
    ...
    conn.commit()
    conn.close()
    print("✅ База данных проверена и обновлена с колонками для чистки снега")
...
def start_snow_work(call):
    try:
        ...
        squares = job_data[0]
        job_end = datetime.strptime(job_data[1], '%Y-%m-%d %H:%M:%S') if isinstance(job_data[1], str) else datetime.fromtimestamp(job_data[1])
        territory = job_data[2]
        
        current_time = datetime.now()
        
        # Проверяем не закончилось ли время
        if current_time > job_end:
            bot.answer_callback_query(call.id, "⏳ Время работы истекло!")
            # Очищаем работу
            cursor.execute('''
                UPDATE users SET 
        ...
        if progress >= squares:
            ...
            result_text += f"📊 Новый баланс: {format_balance(new_balance)}❄️\n\n"
            result_text += f"⏳ Отдых 5 минут до следующей работы"
            ...
            progress_text = "❄️ ИДЕТ УБОРКА СНЕГА\n\n"
            progress_text += f"{territory}\n\n"
            progress_text += f"📊 Прогресс: {progress}/{squares} м²\n"
            progress_text += f"💰 Заработано: {progress * 5}❄️\n"
            progress_text += f"⏱️ Осталось: {minutes_left}:{seconds_left:02d}\n\n"
            progress_text += "Продолжайте нажимать на кнопку!"
            
            # Создаем новую кнопку
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton(f"🗑️ ЧИСТИТЬ ({progress}/{squares})", callback_data="snow_start"))
            
            bot.answer_callback_query(call.id, f"🗑️ +5❄️ ({progress}/{squares})")
            ...
            try:
                ...
                pass  # Если сообщение не изменилось, ничего страшного
