...
def init_db():
    ...
    try:
        ...
        CREATE TABLE IF NOT EXISTS users (
            ...
            display_name TEXT  -- Добавляем колонку для ника
        )
        ''')
        ...
        integrity = cursor.fetchone()[0]
        if integrity == 'ok':
            logging.info("✅ Проверка целолостности БД: OK")
        else:
...
# === ИСПРАВЛЕННАЯ ФУНКЦИЯ ПОЛУЧЕНИЯ ТОПА БАЛАНСОВ ===
def get_balance_top_page(page=1, limit=5):
    ...
    # Исправленный запрос: добавлена колонка display_name
    cursor.execute('''
    SELECT 
        user_id,
        CASE 
            ...
            ELSE COALESCE(first_name, 'Аноним') 
        END as display_name,
        balance,
...
def get_scam_top_page(page=1, limit=5):
    ...
    SELECT 
        ...
        CASE 
            ...
            ELSE COALESCE(u.first_name, 'Аноним') 
        END as display_name,
    ...
    HAVING COUNT(r.user_id) > 0
    ''')
    total_scammers_result = cursor.fetchone()
    total_scammers = total_scammers_result[0] if total_scammers_result else 1
...
# === ИСПРАВЛЕННАЯ ФУНКЦИЯ ПОЛУЧЕНИЯ ПОЗИЦИИ ПОЛЬЗОВАТЕЛЯ ===
...
# === ИСПРАВЛЕННАЯ СОЗДАНИЕ СООБЩЕНИЯ ТОПА ===
...
def handle_top_menu(message):
    try:
        ...
        # Отправляем более простое сообщение об ошибке
        bot.send_message(message.chat.id, "❌ Ошибка загрузки топа. База данных временно недоступна.")

# === ИСПРАВЛЕННЫЙ ОБРАБОТЧИК КОМАНДЫ /name ===
...
def change_name_command(message):
    try:
        ...
        if len(parts) < 2:
            ...
            if 'display_name' not in columns:
                ...
                try:
                    ...
                    conn.commit()
                    logging.info("✅ Создана колонка display_name")
                except Exception as e:
            ...
            if user_data:
                current_nick = user_data[2] if user_data[2] else (f"@{user_data[0]}" if user_data[0] else user_data[1])
            else:
            ...
            help_text += "• `/name [новый_ник]`\n"
            help_text += "• `/nick [новый_ник]`\n"
            help_text += "• `/имя [новый_ник]`\n"
            help_text += "• `/ник [новый_ник]`\n\n"
            help_text += "*📌 Правила ника:*\n"
            help_text += "✅ От 2 до 16 символов\n"
            help_text += "✅ Можно использовать буквы, цифры, пробелы и _\n"
        ...
        import re
        # Разрешаем русские/английские буквы, цифры, пробелы и подчеркивания
        if not re.match(r'^[a-zA-Zа-яА-ЯёЁ0-9_ ]+$', new_name):
            bot.send_message(message.chat.id, 
                           "❌ Ник содержит запрещенные символы!\n"
        ...
        bad_words = [
            ...
            'куратор', 'curator', 'администратор', 'administrator'
        ]
        ...
        # Получаем текущий ник пользователя
        ...
        if username:
            success_text += f"👤 *Старое имя:* @{username}\n"
        ...
        success_text += "🔄 *Чтобы снова изменить:* `/name [ник]`"
        
        bot.send_message(message.chat.id, success_text, parse_mode='Markdown')
        
    except Exception as e:
        logging.error(f"Ошибка в change_name_command: {e}")
        bot.send_message(message.chat.id, "❌ Произошла ошибка при смене ника!")

# === ФУНКЦИЯ ДЛЯ ПРОВЕРКИ И СОЗДАНИЯ КОЛОНКИ display_name ===
def ensure_display_name_column():
    """Создает колонку display_name если её нет"""
    conn = None
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        if 'display_name' not in columns:
            cursor.execute("ALTER TABLE users ADD COLUMN display_name TEXT")
            conn.commit()
            logging.info("✅ Колонка display_name создана")
            return True
            
    except Exception as e:
        logging.error(f"Ошибка создания колонки display_name: {e}")
    finally:
        if conn:
            conn.close()
    return False
