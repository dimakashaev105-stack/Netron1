...
def init_db():
    ...
    required_columns = [
        ...
        'bank_deposit', 'captcha_passed', 'registered_at', 'is_banned', 
        'ban_reason', 'banned_at', 'last_interest_calc'
    ]
    ...
    for column in required_columns:
        if column not in columns:
            if column == 'bank_deposit':
                ...
                cursor.execute(f'ALTER TABLE users ADD COLUMN {column} INTEGER DEFAULT 0')
            elif column == 'captcha_passed':
                cursor.execute(f'ALTER TABLE users ADD COLUMN {column} INTEGER DEFAULT 0')
            elif column == 'is_banned':
                cursor.execute(f'ALTER TABLE users ADD COLUMN {column} INTEGER DEFAULT 0')
            elif column in ['ban_reason', 'banned_at', 'last_interest_calc']:
                if column == 'last_interest_calc':
                    cursor.execute(f'ALTER TABLE users ADD COLUMN {column} TIMESTAMP DEFAULT CURRENT_TIMESTAMP')
                else:
...
# Глобальный обработчик для проверки бана
@bot.message_handler(func=lambda message: True)
def check_ban_before_processing(message):
    try:
        user_id = message.from_user.id
        
        # Проверяем бан
        banned, reason = is_banned(user_id)
        if banned and not message.text.startswith('/'):
            bot.send_message(message.chat.id, f"🚫 Вы забанены в боте!\nПричина: {reason}\nОбратитесь к администратору.")
            return True  # Сообщение обработано
        
        # Проверяем, прошёл ли пользователь капчу (кроме команды /start)
        if message.text and not message.text.startswith('/start'):
            ...
            if user and not is_captcha_passed(user_id):
                bot.send_message(message.chat.id, "🔒 Сначала пройдите капчу! Используйте команду /start")
                return True
        
    except Exception as e:
        print(f"Ошибка в check_ban_before_processing: {e}")
    
    return False  # Продолжить обработку другим обработчикам
...
def start(message):
    try:
        ...
        if not is_captcha_passed(user_id):
            ...
            user_captcha_status[user_id] = captcha_text
            
            markup = InlineKeyboardMarkup()
            markup.add(
                InlineKeyboardButton("1", callback_data=f"captcha_1_{captcha_text}"),
                InlineKeyboardButton("2", callback_data=f"captcha_2_{captcha_text}"),
                InlineKeyboardButton("3", callback_data=f"captcha_3_{captcha_text}"),
                InlineKeyboardButton("4", callback_data=f"captcha_4_{captcha_text}")
            )
            
            bot.send_message(message.chat.id, 
                           f"🔒 Для продолжения введите код с картинки:\n\n"
                           f"Код: **{captcha_text}**\n\n"
                           f"Выберите цифру, которая соответствует картинке ниже:",
                           parse_mode='Markdown',
                           reply_markup=markup)
            ...
            try:
                with open('g.jpg', 'rb') as photo:
                    bot.send_photo(message.chat.id, photo, caption="Картинка для капчи")
            except:
                bot.send_message(message.chat.id, "Картинка для проверки:")
            return
...
def handle_captcha(call):
    try:
        ...
        selected_number = parts[1]
        correct_captcha = parts[2]
        ...
        # Проверяем, соответствует ли выбранная цифра капче
        if correct_captcha in user_captcha_status.get(user_id, ""):
            ...
            markup = create_main_menu()
            bot.send_message(call.message.chat.id, "✅ Регистрация успешно завершена! Добро пожаловать!", reply_markup=markup)
        else:
            bot.answer_callback_query(call.id, "❌ Неверный выбор! Попробуйте еще раз.")
            ...
            bot.edit_message_text(f"❌ Неверный выбор! Попробуйте еще раз:\n\nКод: **{captcha_text}**",
                                call.message.chat.id,
                                call.message.message_id,
                                parse_mode='Markdown',
                                reply_markup=markup)
    except Exception as e:
        print(f"Ошибка в handle_captcha: {e}")
        bot.answer_callback_query(call.id, "❌ Ошибка проверки капчи")
...
def handle_me(message):
    try:
        ...
        if result:
            ...
            message_text += f"🔥 Серия кликов: {click_streak}"
...
def clean_old_captchas():
    while True:
        ...
        current_time = time.time()
        to_remove = []
        for user_id, captcha_time in list(user_captcha_status.items()):
            # Удаляем капчи старше 10 минут
            if current_time - captcha_time > 600:
